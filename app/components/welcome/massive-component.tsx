...
export const AppInfoComp: FC<{ siteInfo: AppInfo }> = ({ siteInfo }) => {
  const description = siteInfo.description || '让销售沟通更清晰、更高效。'
  ...
  return (
    <div className={s.heroIntro}>
      ...
      <h1 className={s.heroTitle}>{siteInfo.title}</h1>
      {description && <p className={s.heroDescription}>{description}</p>}
    </div>
  )
}
...
export const ChatBtn: FC<{ onClick: () => void, className?: string }> = ({
  ...
  return (
    <Button
      type='primary'
      className={cn(className, `space-x-2 flex items-center !h-11 mobile:w-full tablet:!h-11 ${s.customBtn}`)}
      onClick={onClick}
    >
